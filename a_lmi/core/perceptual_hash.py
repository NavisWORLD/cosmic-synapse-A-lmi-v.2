...
class PerceptualHash:
    ...
    def hash_text(self, text: str, num_bits: int = 64) -> str:
        ...
        try:
            ...
            for token in tokens:
                # Legacy MD5 is retained only for deterministic SimHash
                # compatibility; it is not used for a security property.
                h = int(
    ...
    def _fallback_hash(self, data: Union[Image.Image, np.ndarray, bytes, str]) -> str:
        """
        Fallback hash using the legacy deterministic MD5 identifier.

        This digest is used only for deduplication/similarity compatibility,
        never for credentials, signatures, or integrity/security decisions.
        ...
        if isinstance(data, str):
            ...
            return hashlib.md5(
                np.array(data).tobytes(), usedforsecurity=False
            ).hexdigest()
        else:
