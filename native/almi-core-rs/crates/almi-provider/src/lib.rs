...
impl OllamaProvider {
    pub fn new(model_id: &str, endpoint: Option<&str>) -> Result<Self> {
        Self::with_policy(
            model_id,
            endpoint.unwrap_or(DEFAULT_OLLAMA_ENDPOINT),
            None,
            None,
    ...
    pub fn with_policy(
        ...
        if model_id.trim().is_empty() {
            return Err(AlmiError::InvalidInput("model_id must not be empty".into()));
        }
        if timeout.is_zero() {
            return Err(AlmiError::InvalidInput("timeout must be positive".into()));
        }
        let endpoint = endpoint.trim_end_matches('/');
        let parsed = Url::parse(endpoint)
            .map_err(|_| AlmiError::InvalidInput("Ollama endpoint must be a valid URL".into()))?;
        ...
        if parsed.host_str().is_none() {
            return Err(AlmiError::InvalidInput(
                "Ollama endpoint must include a hostname".into(),
            ));
        }
        if !parsed.username().is_empty() || parsed.password().is_some() {
            return Err(AlmiError::InvalidInput(
                "Ollama endpoint must not embed credentials".into(),
            ));
        }
        if parsed.query().is_some() || parsed.fragment().is_some() {
            return Err(AlmiError::InvalidInput(
                "Ollama endpoint must not include a query or fragment".into(),
            ));
        }
        let client = Client::builder()
            .timeout(timeout)
            .build()
            .map_err(|e| AlmiError::Provider(format!("failed to build HTTP client: {e}")))?;
        Ok(Self {
            identity: ProviderIdentity {
    ...
    fn request_json(
        ...
        let base = self
            ...
            .endpoint
            .as_deref()
            .ok_or_else(|| AlmiError::Provider("provider endpoint missing".into()))?;
        ...
        for attempt in 0..=self.retries {
            ...
            match request.send() {
                Ok(response) => match response.error_for_status() {
                    Ok(response) => {
                        return response.json::<Value>().map_err(|_| {
                            AlmiError::Provider("provider returned invalid JSON".into())
                        })
                    }
                    Err(error) => {
            ...
            if attempt < self.retries {
                thread::sleep(Duration::from_millis(
                    (250u64.saturating_mul(1u64 << attempt.min(2))).min(1000),
                ));
            }
        }
        Err(AlmiError::Provider(
        ...
        match self.request_json(reqwest::Method::GET, "/api/tags", None) {
            Ok(value) => {
                let installed = value
                    ...
                    .unwrap_or_default();
                Ok(ProviderHealth {
                    available: installed.iter().any(|name| *name == self.identity.model_id),
                    reachable: Some(true),
    ...
    fn generate(&self, request: &ModelRequest) -> Result<ModelResponse> {
        if request.prompt.trim().is_empty() {
            return Err(AlmiError::Provider(
                "model request prompt must not be empty".into(),
            ));
        }
        let mut payload = serde_json::json!({"model": self.identity.model_id, "prompt": request.prompt, "stream": false});
        if let Value::Object(object) = &mut payload {
            if let Some(system) = &request.system {
                object.insert("system".into(), Value::String(system.clone()));
            }
            if !request.options.is_empty() {
                object.insert("options".into(), serde_json::to_value(&request.options)?);
            }
        }
        ...
        let text = result
            ...
            .ok_or_else(|| {
                AlmiError::Provider("Ollama response does not contain text in 'response'".into())
            })?;
