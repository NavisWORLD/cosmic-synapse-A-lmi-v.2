...
use almi_core::{
    canonical_json_bytes, validate_version, AlmiError, ArtifactManifest, AuthorityPolicy,
    CSTStateEnvelope, ContinuityWorkspace, KnowledgeGraphReference, ProviderProvenance, Result,
    RoutingState, SystemIdentity, COMPONENT_SCHEMA_VERSION, REQUIRED_WORKSPACE_FILES,
...
pub fn initialize_workspace(
    ...
    write_json(
        root.join("knowledge/graph.json"),
    ...
    write_json(
        root.join("provenance/provider.json"),
        &ProviderProvenance::empty(),
    )?;
    write_json(root.join("routing/state.json"), &RoutingState::default())?;
    write_json(
        root.join("policy/authority.json"),
...
pub fn require_workspace(root: &Path) -> Result<()> {
    ...
    for relative in REQUIRED_WORKSPACE_FILES {
        ...
        let meta = fs::symlink_metadata(&path).map_err(|_| {
            AlmiError::Integrity(format!("workspace is missing required file: {relative}"))
        })?;
        if meta.file_type().is_symlink() || !meta.is_file() {
            return Err(AlmiError::Security(format!(
                "required workspace path is not a regular file: {relative}"
            )));
        }
    }
    Ok(())
...
fn read_json<T: DeserializeOwned>(path: PathBuf) -> Result<T> {
    let bytes = fs::read(&path)
        .map_err(|e| AlmiError::Integrity(format!("cannot read {}: {e}", path.display())))?;
    serde_json::from_slice(&bytes)
        .map_err(|e| AlmiError::Integrity(format!("invalid JSON file {}: {e}", path.display())))
}
