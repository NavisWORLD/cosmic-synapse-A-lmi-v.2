...
fn workspace_has_python_compatible_layout_and_deny_defaults() {
    ...
    for path in ["system.json","memory/ledger.jsonl","state/cst.json","knowledge/graph.json","artifacts/manifest.json","provenance/provider.json","routing/state.json","policy/authority.json"] {
...
fn non_empty_destination_is_rejected() {
    ...
    assert_eq!(std::fs::read_to_string(root.join("keep.txt")).unwrap(), "keep");
}
