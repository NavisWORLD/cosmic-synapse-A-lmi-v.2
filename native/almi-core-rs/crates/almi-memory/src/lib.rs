...
pub fn verify_ledger(path: impl AsRef<Path>) -> Result<LedgerVerification> {
    ...
    let text = std::str::from_utf8(&bytes)
        .map_err(|_| AlmiError::Integrity("memory ledger is not valid UTF-8".into()))?;
    ...
    for raw in text.lines() {
        ...
        let record: MemoryRecord = serde_json::from_str(raw).map_err(|e| {
            AlmiError::Integrity(format!("memory ledger contains invalid JSONL: {e}"))
        })?;
        record.validate()?;
