...
fn append_is_canonical_and_verify_preserves_order() {
    ...
    let record = MemoryRecord { version: Some(1), role: "user".into(), content: "sunflower".into(), provider: None, provenance: None, timestamp: None, extra: BTreeMap::new() };
    let first = append_record(&path, &record).unwrap();
    let second = append_record(&path, &MemoryRecord { content: "second".into(), ..record.clone() }).unwrap();
...
fn malformed_or_unknown_version_record_is_rejected() {
    ...
    std::fs::write(&path, "{\"version\":99,\"role\":\"user\",\"content\":\"x\"}\n").unwrap();
    assert!(verify_ledger(&path).is_err());
}
