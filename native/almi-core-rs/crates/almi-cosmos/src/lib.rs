...
use almi_core::{
    canonical_json_bytes, AlmiError, ContinuityManifest, CosmosBundleMetadata, ManifestEntry,
    Result, SystemIdentity, BUNDLE_FORMAT_VERSION, BUNDLE_MANIFEST, MAX_BUNDLE_BYTES,
    MAX_BUNDLE_FILES, REQUIRED_WORKSPACE_FILES, WORKSPACE_FORMAT_VERSION,
};
...
pub fn validate_archive_name(name: &str) -> Result<()> {
    if name.is_empty() || name.starts_with('/') || name.contains('\\') {
        return Err(AlmiError::Security(format!(
...
pub fn export_bundle(
    ...
    let manifest = ContinuityManifest {
        ...
        workspace_name: summary.system.name.clone(),
        files,
    };
    ...
    Ok(CosmosBundleMetadata {
        valid: true,
        path: destination.display().to_string(),
        name: Some(summary.system.name),
        file_count: payloads.len(),
        total_bytes: payloads.iter().map(|(_, v)| v.len() as u64).sum(),
        sha256: digest(&bundle),
    })
}

pub fn inspect_bundle(path: impl AsRef<Path>) -> Result<CosmosBundleMetadata> {
    verify_bundle(path)
}
...
pub fn verify_bundle(path: impl AsRef<Path>) -> Result<CosmosBundleMetadata> {
    ...
    let meta = fs::metadata(source)
        .map_err(|_| AlmiError::Integrity("bundle is not a readable ZIP container".into()))?;
    if meta.len() > MAX_BUNDLE_BYTES.saturating_mul(2) {
        return Err(AlmiError::Security(
            "bundle compressed size is excessive".into(),
        ));
    }
    ...
    if archive.len() > MAX_BUNDLE_FILES + 1 {
        return Err(AlmiError::Security(
            "bundle exceeds portable file-count limit".into(),
        ));
    }
    ...
    for index in 0..archive.len() {
        ...
        if !seen.insert(name.clone()) {
            return Err(AlmiError::Security(
                "bundle contains duplicate archive member names".into(),
            ));
        }
        if member.is_dir() {
            return Err(AlmiError::Security(format!(
                "directory archive member is not allowed: {name}"
            )));
        }
        if member
            .unix_mode()
        ...
        {
            return Err(AlmiError::Security(format!(
                "symlink archive member is not allowed: {name}"
            )));
        }
        if name != BUNDLE_MANIFEST && is_secret_path(&name) {
            return Err(AlmiError::Security(format!(
                "secret-bearing archive member is not allowed: {name}"
            )));
        }
        if member.size() > MAX_BUNDLE_BYTES {
            return Err(AlmiError::Security(format!(
                "archive member is oversized: {name}"
            )));
        }
        total_declared_member_size = total_declared_member_size
            .checked_add(member.size())
            .ok_or_else(|| AlmiError::Security("bundle size overflow".into()))?;
        if total_declared_member_size > MAX_BUNDLE_BYTES {
            return Err(AlmiError::Security(
                "bundle exceeds portable uncompressed-size limit".into(),
            ));
        }
    ...
    let manifest: ContinuityManifest = serde_json::from_slice(&manifest_bytes)
        .map_err(|_| AlmiError::Integrity("bundle manifest is invalid".into()))?;
    ...
    for entry in &manifest.files {
        ...
        if entry.path == BUNDLE_MANIFEST {
            return Err(AlmiError::Integrity(
                "bundle manifest cannot declare itself as payload".into(),
            ));
        }
        if declared.insert(entry.path.clone(), entry).is_some() {
            return Err(AlmiError::Integrity(format!(
    ...
    let system: SystemIdentity = serde_json::from_slice(&system_bytes)
        .map_err(|_| AlmiError::Integrity("bundle system.json is invalid".into()))?;
    system.validate()?;
...
pub fn import_bundle(
    ...
    let mut archive = ZipArchive::new(file).map_err(zip_err)?;
    let manifest: ContinuityManifest =
        serde_json::from_slice(&read_member(&mut archive, BUNDLE_MANIFEST)?)?;
    for entry in &manifest.files {
        validate_archive_name(&entry.path)?;
...
fn collect_payloads(root: &Path) -> Result<Vec<(String, Vec<u8>)>> {
    ...
    if files.len() > MAX_BUNDLE_FILES {
        return Err(AlmiError::Security(
            "workspace exceeds portable bundle file-count limit".into(),
        ));
    }
    let total: u64 = files.iter().map(|(_, b)| b.len() as u64).sum();
    if total > MAX_BUNDLE_BYTES {
        return Err(AlmiError::Security(
            "workspace exceeds portable bundle size limit".into(),
        ));
    }
    Ok(files)
}
...
fn visit(root: &Path, dir: &Path, out: &mut Vec<(String, Vec<u8>)>) -> Result<()> {
    ...
    for entry in entries {
        ...
        let relative = path
            .strip_prefix(root)
            .map_err(|_| AlmiError::Security("workspace path escaped root".into()))?
            .to_string_lossy()
            .replace('\\', "/");
        if meta.file_type().is_symlink() {
            return Err(AlmiError::Security(format!(
                "workspace symlink is not portable: {relative}"
            )));
        }
        if meta.is_dir() {
            visit(root, &path, out)?;
        ...
        if is_secret_path(&relative) {
            return Err(AlmiError::Security(format!(
                "secret-bearing file is excluded from bundles: {relative}"
            )));
        }
        let bytes = fs::read(&path)?;
        if bytes.len() as u64 > MAX_BUNDLE_BYTES {
            return Err(AlmiError::Security(format!(
                "workspace file is oversized: {relative}"
            )));
        }
...
fn zip_err(error: zip::result::ZipError) -> AlmiError {
    AlmiError::Integrity(format!("ZIP error: {error}"))
}
