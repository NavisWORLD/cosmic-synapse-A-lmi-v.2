...
impl ModelProvider for DeterministicProvider {
    ...
    fn health(&self) -> Result<almi_core::ProviderHealth> {
        Ok(almi_core::ProviderHealth {
            available: true,
            reachable: Some(true),
...
fn p(id: &str, model: &str, prefix: &'static str) -> DeterministicProvider {
    DeterministicProvider {
        identity: ProviderIdentity {
            provider_id: id.into(),
            model_id: model.into(),
            revision: Some("test".into()),
            endpoint: Some("test://local".into()),
...
fn provider_swap_keeps_continuity_and_exact_authority_bytes() {
    ...
    assert_eq!(
        runtime.interact("continue", None).unwrap().text,
        "B:continue"
    );
    assert_eq!(
        std::fs::read(root.join("policy/authority.json")).unwrap(),
        authority_before
    );
