...
fn execute(cli: &Cli) -> Result<Value> {
    match &cli.command {
        Command::Doctor => Ok(
            json!({"status":"ok","version":env!("CARGO_PKG_VERSION"),"abi_version":ABI_VERSION,"authority_default":"deny","network_default":"loopback provider default; no network on construction","note":"software health only"}),
        ),
        Command::Version => {
            Ok(json!({"version":env!("CARGO_PKG_VERSION"),"abi_version":ABI_VERSION}))
        }
        Command::Init {
            workspace,
            name,
            seed,
        } => Ok(serde_json::to_value(initialize_workspace(
            workspace, name, *seed,
        )?)?),
        Command::Inspect { workspace } => Ok(serde_json::to_value(inspect_workspace(workspace)?)?),
        Command::Export { workspace, bundle } => {
            Ok(serde_json::to_value(export_bundle(workspace, bundle)?)?)
        }
        Command::Verify { bundle } => Ok(serde_json::to_value(verify_bundle(bundle)?)?),
        Command::Import { bundle, workspace } => {
            Ok(serde_json::to_value(import_bundle(bundle, workspace)?)?)
        }
        Command::Memory {
            command: MemoryCommand::Verify { workspace },
        } => {
...
fn provider_command(command: &ProviderCommand) -> Result<Value> {
    match command {
        ProviderCommand::List => Ok(
            json!({"network_checked":false,"providers":[{"provider_id":"ollama","default_endpoint":DEFAULT_OLLAMA_ENDPOINT,"capabilities":["text"]}]}),
        ),
        ...
        ProviderCommand::Run {
            ...
            Ok(serde_json::to_value(provider.generate(
                &almi_core::ModelRequest {
                    prompt: prompt.clone(),
                    system: system.clone(),
    ...
    match command {
        RuntimeCommand::Run {
            ...
            Ok(serde_json::to_value(
                runtime.interact(prompt, system.as_deref())?,
            )?)
        }
    }
}
